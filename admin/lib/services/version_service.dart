...
class RemoteVersion {
  ...
  String get downloadUrl {
    ...
    return 'https://makazs.xyz${u.startsWith('/') ? '' : '/'}$u';
  }
}

class VersionService {
  /// 当前管理面板版本（与 pubspec.yaml version 保持一致）
  static const currentVersion = '0.26.9';
  static const segment = 'admin';
  ...
  static Future<RemoteVersion?> fetch() async {
    try {
      final res = await http
          .get(Uri.parse('https://makazs.xyz/downloads/version.json'))
  ...
  static Future<bool> isSkipped(RemoteVersion v) async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getString('skip_version_$segment') == v.version;
  }

  static Future<void> markSkipped(RemoteVersion v) async {
    final prefs = await SharedPreferences.getInstance();
