...
class UpdateDialog {
        ...
        child: AlertDialog(
          ...
          actions: [
            ...
            if (!force)
              TextButton(
                onPressed: () async {
                  await VersionService.markSkipped(segment, info);
  ...
  static Future<bool> check(
    ...
    final v = await VersionService.fetch(segment: segment);
    if (v == null || !VersionService.hasUpdate(currentVersion, v)) {
      if (manual && context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('当前已是最新版本')));
      }
      return false;
    }
    final force = VersionService.isForced(currentVersion, v);
    if (!force && !manual && await VersionService.isSkipped(segment, v)) return false;
    if (context.mounted) await show(context, info: v, force: force, segment: segment);
    return true;
  }
}
